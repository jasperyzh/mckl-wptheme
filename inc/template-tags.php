...
if (!function_exists('twenty_twenty_one_posted_on')) {
	...
	function twenty_twenty_one_posted_on()
	{
		...
		printf(
			...
			'<span class="badge">Published on </span> %s',
		...
		if (!is_single()) {
			...
			if (has_category() || has_tag()) {
				...
				if ($categories_list) {
					...
					printf(
						/* translators: %s: List of categories. */
						'<span class="badge mr-2">' . esc_html__('Category &mdash; %s', 'twentytwentyone') . ' </span>',
						$categories_list // phpcs:ignore WordPress.Security.EscapeOutput
					);
				}
				...
				if ($tags_list) {
					printf(
						...
						/* translators: %s: List of tags. */
						'<span class="badge badge__tag mr-2">' . esc_html__('Tag &mdash; %s', 'twentytwentyone') . '</span>',
						$tags_list // phpcs:ignore WordPress.Security.EscapeOutput
					);
				}
				echo '</div>';
			...
			if (has_category() || has_tag()) {
				...
				if ($categories_list) {
					...
					printf(
						/* translators: %s: List of categories. */
						'<span class="badge badge__category mr-2">' . esc_html__('Category &mdash; %s', 'twentytwentyone') . ' </span>',
						$categories_list // phpcs:ignore WordPress.Security.EscapeOutput
					);
				}
...
if (!function_exists('mckl_entry_meta_header')) {
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 * Footer entry meta is displayed differently in archives and single posts.
	 *
	 ...
	 * @return void
	 */
	function mckl_entry_meta_header()
	{
		...
		if (!is_single() && false) {
			...
			if (has_category() /* || has_tag() */) {
				...
				echo '<div class="edit d-inline-block">';
				// Posted on.
				// twenty_twenty_one_posted_on();
				// Posted by.
				// twenty_twenty_one_posted_by();
